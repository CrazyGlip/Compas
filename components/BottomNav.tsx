...
interface BottomNavProps {
    activeTab: 'dashboard' | 'shorts' | 'news';
    onSelect: (tab: 'dashboard' | 'shorts' | 'news') => void;
}
    ...
    return (
        <div className="fixed bottom-0 left-0 right-0 bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-t border-slate-200 dark:border-white/10 h-16 flex justify-around items-center z-50 pb-safe transition-colors duration-300">
            <button 
                ...
                className={`flex flex-col items-center justify-center w-full h-full transition-colors ${activeTab === 'dashboard' ? 'text-sky-600 dark:text-sky-400' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300'}`}
            >
            ...
            <button 
                ...
                className={`flex flex-col items-center justify-center w-full h-full transition-colors ${activeTab === 'shorts' ? 'text-fuchsia-600 dark:text-fuchsia-500' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300'}`}
            >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7 mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
