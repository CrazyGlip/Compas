...
// DNA Bar Component for the new tab
...
const ProfessionDetailScreen: React.FC<ProfessionDetailScreenProps> = ({ specialtyId, onBack, onNavigateToCollege, isInPlan, onAddToPlan, onRemoveFromPlan, isAdminMode }) => {
    ...
    // Sort tags by weight descending for DNA tab
    const weightedSpecs = (specialty.specs || []).sort((a, b) => b.weight - a.weight);
    ...
    const renderTabContent = () => {
        switch (activeTab) {
            ...
            case 'dna':
                return (
                    <div className="space-y-6 animate-fade-in">
                        <div className="bg-slate-50 dark:bg-slate-900/40 p-6 rounded-2xl border border-slate-200 dark:border-white/5">
                            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
                                <span className="text-2xl">🧬</span> ДНК Профессии
                            </h3>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">
                                Эти характеристики показывают, насколько данная специальность совпадает с различными типами личности и интересами.
                            </p>
                            ...
                            {weightedSpecs.length > 0 ? (
                                ...
                                <p className="text-slate-400 text-center py-4">Характеристики не заданы.</p>
                            )}
                        </div>
                    </div>
                );
        ...
        <div className="space-y-6 animate-fade-in pb-10 relative">
            ...
            <div className="bg-white dark:bg-slate-800/50 backdrop-blur-xl rounded-2xl p-6 shadow-2xl border border-slate-200 dark:border-white/10">
                <div className="flex flex-wrap gap-2 mb-6 border-b border-slate-200 dark:border-white/10 pb-4">
                    ...
                    <TabButton isActive={activeTab === 'dna'} onClick={() => setActiveTab('dna')}>ДНК</TabButton>
